...
class DiscordXp {
  ...
  */
  ...
  /**
  * @param {string} [bot] - Your Discord.CLient.
  * @param {array} [leaderboard] - The output from 'fetchLeaderboard' function.
  */

  static computeLeaderboard(bot, leaderboard) {
    if (!bot) throw new TypeError("A client was not provided.");
    ...
    leaderboard.map(key => computedArray.push({
        ...
        username: bot.users.cache.get(key.userID) ? bot.users.cache.get(key.userID).username : "Unknown",
        discriminator: bot.users.cache.get(key.userID) ? bot.users.cache.get(key.userID).discriminator : "0000"
      }));
